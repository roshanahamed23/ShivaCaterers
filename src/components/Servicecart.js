...
const Servicecart = ({ icon, title, description, buttonText, link }) => {
  return (
    <div className="border rounded-md shadow-sm text-center p-6 flex flex-col gap-4 justify-center items-center">
      ...
      <p className="font-monserrat text-black max-w-sm">{description}</p>
      <a href={link}>
        <button className="border border-black px-5 py-3 rounded-lg text-black font-montserrat transition duration-300 ease-in-out transform cursor-pointer hover:scale-105">
          {buttonText}
        </button>
      </a>
    </div>
